...
function autoBind<T extends object>(instance: T): void {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    if (key !== 'constructor' && typeof instance[key as keyof T] === 'function') {
      const value = instance[key as keyof T] as unknown;
      if (typeof value === 'function') {
        (instance[key as keyof T] as unknown) = value.bind(instance);
      }
    }
  });
}
...
class App {
  ...
  handleClick(e: MouseEvent | TouchEvent) {
    ...
    this.mouse.x = 2.0 * (x - rect.left) / rect.width - 1.0;
    this.mouse.y = 2.0 * (1.0 - (y - rect.top) / rect.height) - 1.0;
    this.raycast.castMouse(this.camera, this.mouse);
    const meshes = this.medias.map((m) => m.plane);
    const hits = this.raycast.intersectBounds(meshes);
    if (hits.length) {
      const hitMesh = hits[0];
      const index = this.medias.findIndex((m) => m.plane === hitMesh);
      if (index !== -1) this.onItemClick(index % (this.mediasImages.length / 2));
    }
  }
...
export default function CircularGallery({
  ...
  return <div className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing" ref={containerRef} ></div>;
}
